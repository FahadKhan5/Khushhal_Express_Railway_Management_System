...
class booking:public passenger
{
	...
	bool ldest()
	{
		...
		cout<<endl<<"These are the local destinations where we are Travelling: \n";
		for(i=0;i<5;i++)
		{
			for(j=0;j<20;j++)
			{
	...
	{
		passport = pass;
		cout<<endl<<"These are the international destinations where we are Travelling: \n";
		for(i=0;i<11;i++)
		{
			for(j=0;j<20;j++)
			{
		...
		bool confirm()
		{
			...
			if(ch=='Y'||ch=='y')
			{
				...
				ofstream outfile;
				outfile.open("travel.txt",ios::app);
				...
				if(y==v)
				{
					...
					switch(option)
					{
						...
						case 50:
						{
							file.close();
							ofstream ofli;
							ofli.open("travel.txt",ios::trunc);
							ofli.close();
							remove("travel.txt");
							system("cls");
...
class about
{
	public:
		void Aboutus()
		{
			cout<<"Since 1980 we are serving our train services and day by day we are making it excellent. We have 4 number of Trains."<<endl<<endl;
...
class complain
{
	...
	void type()
	{
		...
		if(choice==49)
		{
			cout<<"Railway Handling: ";
			types = "Railway Handling: ";
		}
		else if(choice==50)
		{
...
class routes{
	public:
		bool print()
		{
			int i,j;
			char ch;
			cout<<"\n\n\n\n\tFollowing are the international Travels available"<<endl;
			for(i=0;i<11;i++)
			{
				cout<<endl;
				for(j=0;j<11;j++)
				{
					...
					{
						cout<<"Travel From "<<i_destination[i]<<" to "<<i_destination[j]<<endl;
					}
					
				}
			}
			cout<<"\n\n\n\n\tFollowing are the local travels available"<<endl;
			for(i=0;i<5;i++)
			{
				cout<<endl;
				for(j=0;j<5;j++)
				{
					...
					{
						cout<<"Travel from "<<l_destination[i]<<" to "<<l_destination[j]<<endl;
					}
				}
			}
			cout<<endl<<"Do you want to book a Seat?(y/n) ";
			ch=getch();
			if(ch=='Y'||ch=='y')
			{
				cout<<endl<<endl;
...
int main()
{
	...
	cout<<endl<<"\n\n\n\tWelcome to Khushhal Express"<<endl;
...
//				}
				...
				if(choice==49)
				{
					...
					if(ptr->ldest()==false)
					{
						cout<<endl<<"We donot travel to this place\nPlease Choose again\n";
						goto back;
					}
					else
					{
						cout<<endl<<"Yes we do travel\n";
						ptr->seats();
						ptr->Meals();
						ptr->Registration();
						system("cls");
						...
						{
							cout<<endl<<"Thanks for Travelling With Us"<<endl<<endl
